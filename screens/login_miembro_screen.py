...
class LoginMiembroScreen(Screen):
    def __init__(self, **kwargs):
        super(LoginMiembroScreen, self).__init__(**kwargs)
        self.name = 'login_miembro'
        self.db = Database()
        self.mostrar_login()
    ...
    def mostrar_registro(self):
        ...
        # Título
        title = Label(
            text='Registro de Nuevo Miembro',
            font_size='36sp',
            color=(0.95, 0.95, 0.90, 1),  # Crema/blanco
            bold=True,
            size_hint=(1, 0.12),
    ...
    def login(self, instance):
        ...
        resultado = Miembro.login(collection, email, password)

        if resultado['success']:
            # Ir al dashboard del miembro
            dashboard_screen = self.manager.get_screen('dashboard_miembro')
            dashboard_screen.set_usuario(resultado['usuario'])
            self.manager.current = 'dashboard_miembro'
        else:
            self.mostrar_popup("Error", resultado['message'])

    def registrar(self, instance):
        """Maneja el registro de nuevo miembro"""
        ...
        collection = self.db.get_collection('miembros')
        resultado = Miembro.crear(collection, nombre, email, password)
